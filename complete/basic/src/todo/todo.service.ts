...
export class TodoService {
  ...
  /**
   * Create a new Todo item with auto-generated sequence number
   * This publishes a command to DynamoDB which will be processed by Step Functions
   *
   * Key addition in this step:
   * - genNewSequence() generates a unique sequence number per tenant
   * - The sequence number can be used as a human-readable ID (e.g., TODO-0001)
   */
  async create(
    ...
    // Generate a new sequence number for this tenant
    // Format: [tenantCode]-[4-digit sequence]
    // Example: MBC-0001, MBC-0002, ...
    ...
    // Create command DTO with sequence number
    const todo = new TodoCommandDto({
      ...
      seq: seqItem.no, // Add sequence number to the entity
      name: createDto.name,
